    ...
    while not done:
        ...
        for c in c_list:  
            ...
            if not curr_c.attrs['State']['Running'] or curr_c.attrs['State']['Health']['Status'] != 'healthy':                   
                ...
                logging.debug("state: {}".format(curr_c.attrs['State']['Status']))
                logging.debug("health: {}".format(curr_c.attrs['State']['Health']['Status']))
                done = False
...
def killswitch(client: docker.DockerClient, target_level: int): 
    ...
    min_level, max_level, active_level = find_current_level_stats(client.containers.list(all=True, sparse=True))
    logging.info("Min/max/active level: {}/{}/{}".format(min_level, max_level, active_level))

    # group the containers by their level
    grouped_c_lists = group_by_level(client.containers.list(all=True, sparse=True))

    # start from lower -> higher levels
    if target_level >= active_level: 
        if target_level > max_level:
            logging.info("Target level above maximum available level. Verifying containers are started.")
        
        if target_level == active_level:        
            logging.info("Target level is equal to current level. Verifying containers are started.")
        
        for level in sorted(grouped_c_lists.keys()):            
            res = [do_killswitch(c, target_level) for c in grouped_c_lists[level]]
            ...
            for k,v in res:
                logging.info("container: {}    state: {}".format(k.attrs['Names'][0], v))
